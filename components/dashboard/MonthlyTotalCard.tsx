...
export default function MonthlyTotalCard({ viewerId }: { viewerId: string }) {
  ...
  const loadData = async () => {
    ...
    const { data } = await supabase.from('sessions')
      .select('duration_min').eq('user_id', viewerId)
      .gte('actual_start', start.toISOString()).lte('actual_end', end.toISOString());
    const sum = (data || []).reduce((a, s) => a + (s.duration_min ?? 0), 0);
    setTotalMin(sum);
  };
  ...
  return (
    <Card><CardContent className="p-4 text-center">
      <h3 className="font-semibold mb-2">📆 이번 달 총 공부시간</h3>
      <p className="text-3xl font-bold text-indigo-600">{h}시간 {m}분</p>
    </CardContent></Card>
  );
}
